...
const CreateMission = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setInterestCategories(categoriesRes.data || []);
      ...
      // Set default values
      if (types.length > 0) {
        setValue("missionTypeId", types[0].id.toString());
      }
      if (categories.length > 0) {
        setValue("interestCategoryId", categories[0].id.toString());
      }
    } catch (error) {
  ...
  return (
    <div className="p-6 space-y-6">
      <AppBreadcrumb paths={["admin", "manage-missions", "create"]} />
          ...
          <CardContent className="space-y-4">
            ...
            {fields.length === 0 && (
              <div className="text-center py-6 text-gray-500 bg-gray-50 rounded-lg border-2 border-dashed">
                No condition rules. Click "Add Rule" below to create conditions (optional).
              </div>
            )}
